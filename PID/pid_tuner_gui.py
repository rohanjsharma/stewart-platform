...
class PIDTunerGUI:
    ...
    def create_gui(self):
        ...
        # X-Axis PID Section
        x_frame = ttk.LabelFrame(self.root, text="X-Axis PID", padding=20)
        x_frame.pack(pady=15, padx=30, fill=tk.X)
        
        # Kp_X
        ttk.Label(x_frame, text="Kp_X (Proportional)", font=("Arial", 14)).pack(pady=(0, 5))
        ...
        kp_x_slider.pack(pady=5, fill=tk.X)
        self.kp_x_label = ttk.Label(x_frame, text=f"Kp_X: {self.pid.Kp_x:.2f}", font=("Arial", 12))
        self.kp_x_label.pack(pady=(0, 10))
        
        # Ki_X
        ttk.Label(x_frame, text="Ki_X (Integral)", font=("Arial", 14)).pack(pady=(0, 5))
        self.ki_x_var = tk.DoubleVar(value=self.pid.Ki_x)
        ki_x_slider = ttk.Scale(x_frame, from_=0, to=10, variable=self.ki_x_var,
                               orient=tk.HORIZONTAL, length=500)
        ki_x_slider.pack(pady=5, fill=tk.X)
        self.ki_x_label = ttk.Label(x_frame, text=f"Ki_X: {self.pid.Ki_x:.2f}", font=("Arial", 12))
        self.ki_x_label.pack(pady=(0, 10))
        
        # Kd_X
        ttk.Label(x_frame, text="Kd_X (Derivative)", font=("Arial", 14)).pack(pady=(0, 5))
        self.kd_x_var = tk.DoubleVar(value=self.pid.Kd_x)
        kd_x_slider = ttk.Scale(x_frame, from_=0, to=20, variable=self.kd_x_var,
                               orient=tk.HORIZONTAL, length=500)
        kd_x_slider.pack(pady=5, fill=tk.X)
        self.kd_x_label = ttk.Label(x_frame, text=f"Kd_X: {self.pid.Kd_x:.2f}", font=("Arial", 12))
        self.kd_x_label.pack(pady=(0, 10))
        
        # Y-Axis PID Section
        y_frame = ttk.LabelFrame(self.root, text="Y-Axis PID", padding=20)
        y_frame.pack(pady=15, padx=30, fill=tk.X)
        
        # Kp_Y
        ttk.Label(y_frame, text="Kp_Y (Proportional)", font=("Arial", 14)).pack(pady=(0, 5))
        self.kp_y_var = tk.DoubleVar(value=self.pid.Kp_y)
        kp_y_slider = ttk.Scale(y_frame, from_=0, to=50, variable=self.kp_y_var,
                               orient=tk.HORIZONTAL, length=500)
        kp_y_slider.pack(pady=5, fill=tk.X)
        self.kp_y_label = ttk.Label(y_frame, text=f"Kp_Y: {self.pid.Kp_y:.2f}", font=("Arial", 12))
        self.kp_y_label.pack(pady=(0, 10))
        
        # Ki_Y
        ttk.Label(y_frame, text="Ki_Y (Integral)", font=("Arial", 14)).pack(pady=(0, 5))
        self.ki_y_var = tk.DoubleVar(value=self.pid.Ki_y)
        ki_y_slider = ttk.Scale(y_frame, from_=0, to=10, variable=self.ki_y_var,
                               orient=tk.HORIZONTAL, length=500)
        ki_y_slider.pack(pady=5, fill=tk.X)
        self.ki_y_label = ttk.Label(y_frame, text=f"Ki_Y: {self.pid.Ki_y:.2f}", font=("Arial", 12))
        self.ki_y_label.pack(pady=(0, 10))
        
        # Kd_Y
        ttk.Label(y_frame, text="Kd_Y (Derivative)", font=("Arial", 14)).pack(pady=(0, 5))
        self.kd_y_var = tk.DoubleVar(value=self.pid.Kd_y)
        kd_y_slider = ttk.Scale(y_frame, from_=0, to=20, variable=self.kd_y_var,
                               orient=tk.HORIZONTAL, length=500)
        kd_y_slider.pack(pady=5, fill=tk.X)
        self.kd_y_label = ttk.Label(y_frame, text=f"Kd_Y: {self.pid.Kd_y:.2f}", font=("Arial", 12))
        self.kd_y_label.pack(pady=(0, 10))
        ...
        reset_btn = ttk.Button(button_frame, text="Reset Integrals", 
                              command=self.reset_integrals, width=18)
        reset_btn.pack(side=tk.LEFT, padx=10)
        
        close_btn = ttk.Button(button_frame, text="Close", 
                              command=self.close_gui, width=15)
